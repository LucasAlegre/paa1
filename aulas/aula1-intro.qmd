...
## O algoritmo guloso é ótimo para qualquer sistema de moedas?
...
## Para que conjuntos de moedas o algoritmo guloso é ótimo?

> [**Teorema.**]{.label-thm} Quando cada moeda é um **múltiplo inteiro da moeda imediatamente menor** e $c_1 = 1$, o algoritmo do caixa é **ótimo**.

[**Prova.**]{.label-prf}
Suponha um sistema de moedas $c_1, c_2, ..., c_n$. onde $c_1=1$ e cada moeda $c_{i+1} = c_i \cdot k_i$ para algum inteiro $k_i \ge 2$.

:::{.incremental}
- Logo, nenhuma solução ótima pode usar $\ge k_i$ moedas de cada tipo $c_i$, pois, se usasse, poderíamos trocar $k_i$ moedas de tipo $c_i$ por uma única moeda de tipo $c_{i+1}$, o que resultaria em um número menor de moedas, contradizendo a suposição de que a solução é ótima.

- Portanto, em uma solução ótima, a quantidade máxima permitida de qualquer moeda $c_i$ é sempre $k_i-1$.

- Essa é uma condição **suficiente**, não necessária, para que o algoritmo guloso seja ótimo. Não existe uma única propriedade matemática simples e fechada que defina todos os sistemas de moedas canônicos.
:::

## Conjuntos de Moedas Não Canônicos e Programação Dinâmica

- Em 2005, David Pearson publicou um algoritmo que consegue verificar se qualquer conjunto de moedas é canônico em tempo polinomial $O(n^3)$.

- Para sistemas de moedas não canônicos, existe algoritmo ótimo que utiliza programação dinâmica (PAA II):

[Algoritmo de Programação Dinâmica](https://github.com/BrunoGrisci/projeto-e-analise-de-algoritmos/blob/main/paa2/troco.py)
