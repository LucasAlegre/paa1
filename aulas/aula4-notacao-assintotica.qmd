...
$$
\begin{align}
T(n) &= c_1 n + c_2(n-1) + 0(n-1) + c_4(n-1) + c_5 \sum_{j=2}^n 1 + \\ 
...
::: {.incremental}
- $\textcolor{#7c3aed}{a}$, $\textcolor{#7c3aed}{b}$ e $\textcolor{#7c3aed}{c}$ dependem do custo das instruções ($c_i$).
- $T(n) = n^2$?
...
> [**Def.**]{.label-def} $T(n)$ é $O(f(n))$ se e somente se existe uma constante $c > 0$ e um $n_0 \geq 0$ tal que $T(n) \leq c \cdot f(n)$ para todo $n \geq n_0$.
...
> [**Obs.**]{.label-rmk} $c$ e $n_0$ não podem depender de $n$.
...
$T(n)$ é $O(n)$: $c = 4$ e $n_0 = 1$, $c = 3{,}5$ e $n_0 = 2$, $\dots$
...
> [**Def.**]{.label-def} $T(n)$ é $O(f(n))$ se e somente se existe uma constante $c > 0$ e um $n_0$ tal que $T(n) \leq c \cdot f(n)$ para todo $n \geq n_0$.


> [**Afirmação.**]{.label-thm} Se $T(n) = a_k n^k + \dots + a_1 n^1 + a_0$ então $T(n) = O(n^k)$.

. . .

> [**Prova.**]{.label-prf} Escolha $n_0=1$ e $c=|a_k|+\dots+|a_1|+|a_0|$.
