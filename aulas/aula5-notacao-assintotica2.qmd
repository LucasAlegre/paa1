...
> [**Obs.**]{.label-rmk} Seja $T(n)$ uma função em $n=1, 2, 3, \dots$ (em geral o pior tempo de execução de um algoritmo).
...
> 
::: {.incremental}
- É dado que $\textcolor{blue}{f}(n) \leq c' \textcolor{orange}{g}(n)$ **para todo** $n \geq n_0'$ e $\textcolor{orange}{g}(n) \leq c'' \textcolor{#0284c7}{h}(n)$ **para todo** $n \geq n_0''$.
- Considere $n_0 = \max(n_0', n_0'')$, então nós temos que $\textcolor{blue}{f}(n) \leq c' \textcolor{orange}{g}(n) \leq c' c'' \textcolor{#0284c7}{h}(n)$.
- Logo, $\textcolor{blue}{f}(n) \leq c' c'' \textcolor{#0284c7}{h}(n)$ e $\textcolor{blue}{f}(n) = O(\textcolor{#0284c7}{h}(n))$. $\blacksquare$
:::
...
- Dividindo por $\textcolor{red}{c}$: $\textcolor{orange}{g}(n) \geq \frac{1}{\textcolor{red}{c}} \cdot \textcolor{blue}{f}(n)$ **para todo** $n \geq \textcolor{#2e7d32}{n_0}$.
...
> 
::: {.incremental}
- É dado que $\textcolor{blue}{f}(n) \leq c' \textcolor{#0284c7}{h}(n)$ **para todo** $n \geq n_0'$ e $\textcolor{orange}{g}(n) \leq c'' \textcolor{#0284c7}{h}(n)$ **para todo** $n \geq n_0''$.
- Considere $n_0 = \max(n_0', n_0'')$, então nós temos que $\textcolor{blue}{f}(n) + \textcolor{orange}{g}(n) \leq c' \textcolor{#0284c7}{h}(n) + c'' \textcolor{#0284c7}{h}(n) = (c' + c'') \textcolor{#0284c7}{h}(n)$.
...
> 
::: {.incremental}
- $\textcolor{blue}{f}(n) + \textcolor{orange}{g}(n) = \Omega(\textcolor{blue}{f}(n))$ já que $\textcolor{blue}{f}(n) + \textcolor{orange}{g}(n) \geq \textcolor{blue}{f}(n)$.
- Já que $\textcolor{orange}{g}(n) = O(\textcolor{blue}{f}(n))$ e $\textcolor{blue}{f}(n) = O(\textcolor{blue}{f}(n))$, então $\textcolor{blue}{f}(n) + \textcolor{orange}{g}(n) = O(\textcolor{blue}{f}(n))$.
...
- **O que é?** Algoritmo quântico de busca não estruturada, criado por Lov Grover em 1996.
- **Ideia principal:** Dado um domínio de tamanho $N$, encontra (com alta probabilidade) a entrada que leva a um valor de saída desejado usando apenas $O(\sqrt{N})$ avaliações da função.
- **Comparação com computação clássica:** O problema análogo exige $O(N)$ avaliações.
